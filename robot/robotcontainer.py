...
class RobotContainer:
    ...
    def mapButton(
        self, button, stateTrigger
    ):  # Maps a physical button to trigger a state change
        commands2.button.JoystickButton(self.driverController, button).onTrue(
            commands2.cmd.runOnce(lambda: self.state.handleButton(stateTrigger, True))
        ).onFalse(
            commands2.cmd.runOnce(lambda: self.state.handleButton(stateTrigger, False))
        )

    def mapPOV(
        self, angle, stateTrigger
    ):  # Maps a physical button to trigger a state change
        commands2.button.POVButton(self.driverController, angle).onTrue(
            commands2.cmd.runOnce(lambda: self.state.handleButton(stateTrigger, True))
        ).onFalse(
            commands2.cmd.runOnce(lambda: self.state.handleButton(stateTrigger, False))
        )
    ...
    def mapEndstop(self, port, stateTrigger, isInverted=False):
        commands2.button.Trigger(wpilib.DigitalInput(port).get).onTrue(
            commands2.cmd.runOnce(
                lambda: self.state.handleButton(stateTrigger, not isInverted)
            )
        ).onFalse(
            commands2.cmd.runOnce(
                lambda: self.state.handleButton(stateTrigger, isInverted)
            )
        )
    ...
    def configureButtonBindings(self):
        ...
        # Shuffleboard
        commands2.button.Trigger(self.state.isEndstopOverride).onTrue(
            commands2.cmd.runOnce(lambda: print("!!!ENDSTOPS OVERRIDDEN!!!"))
        )
        ...
        commands2.button.Trigger(self.state.isRetracting).whileTrue(
            Retract(self.bucket)
        )
        ...
        commands2.button.JoystickButton(self.driverController, 5).onTrue(
            commands2.cmd.runOnce(lambda: self.drive.FOReset(), self.drive)
        )
